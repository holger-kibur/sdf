...
void main() {
    uint thing;
    map(Initial_Ray_Position, thing);
    vec3 norm = calcSphereNormal(Initial_Ray_Position, thing);
    float intensity = max(0, dot(normalize(-Initial_Ray_Position), norm));
...
fn cam_rotator(
    ...
    let z = (time.seconds_since_startup() / 2.0).cos() * (cam_dist.0 as f64);
    let x = (time.seconds_since_startup() / 2.0).sin() * (cam_dist.0 as f64);
...
fn setup(
    ...
    for _ in 0..16 {
        ...
        .spawn_bundle(MeshBundle {
            mesh: meshes.add(Mesh::from(shape::Icosphere { radius: radius, subdivisions: 5})),
